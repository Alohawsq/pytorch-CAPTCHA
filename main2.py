...
class CNN_Network(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            ...
            nn.MaxPool2d(stride=1, kernel_size=1),                 # 16,90,35
        )
        ...
        self.layer4 = nn.Sequential(
            nn.Conv2d(128,256, stride=1, kernel_size=3, padding=1),  # 256,22,8
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.Conv2d(256,512, kernel_size=3, stride=1, padding=1),  # 512,22,8
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Conv2d(512,1024, stride=1, kernel_size=3, padding=1),  # 1024,22,8
            nn.BatchNorm2d(1024),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # 1024,11,4
        )
        self.fc = nn.Sequential(
            nn.Linear(45056, 2048),
            nn.ReLU(inplace=True),
            nn.Linear(2048, 1024),
            nn.ReLU(inplace=True),
            nn.Linear(1024, 248)
